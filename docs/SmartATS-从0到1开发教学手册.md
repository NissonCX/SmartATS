...
### 6.3 开发顺序（⚠️ 重要！必须按顺序执行）

> **经验教训：先打开"门"，再进"屋"**
>
> 很多初学者会犯的错误：先写业务代码，然后发现所有请求都被 Spring Security 拦截返回 401。
>
> **原因**：Spring Boot 项目引入 `spring-boot-starter-security` 依赖后，默认会拦截所有 HTTP 请求。
>
> **正确做法**：先配置 Spring Security，允许注册/登录接口匿名访问，然后再写业务逻辑。

#### 正确的开发顺序（严格按照这个顺序！）

```
第 1 步：配置 Spring Security ⭐ 最重要！
  └─ 创建 SecurityConfig.java
  └─ 允许 /api/v1/auth/register 和 /api/v1/auth/login 匿名访问
  └─ 其他接口需要认证（后续实现 JWT 过滤器）
  └─ 验证：启动应用，访问这些接口不会返回 401
...
第 3 步：实现 UserService（业务逻辑）
  └─ register() 方法
  └─ login() 方法
  └─ 使用 MyBatis-Plus 操作数据库
  └─ 验证：单元测试通过

第 4 步：实现 AuthController（接口）
  └─ POST /api/v1/auth/register
  └─ POST /api/v1/auth/login
  └─ 验证：用 Postman/curl 测试成功
...
```
❌ 第 1 步：写 UserService（业务逻辑）
❌ 第 2 步：写 AuthController（接口）
❌ 第 3 步：测试接口 → 401（被 Spring Security 拦截）
❌ 第 4 步：发现需要配置 Security（回头修改）
```

**问题**：
- 写完业务代码无法测试（所有请求返回 401）
- 开发体验差，不知道是代码问题还是 Security 配置问题
- 容易让人失去信心

#### 第 1 步详解：配置 Spring Security

**文件位置**：`src/main/java/com/smartats/config/SecurityConfig.java`
...
**验收标准**：
- ✅ 启动应用后，访问 `http://localhost:8080/api/v1/auth/register` 不会返回 401
- ✅ 即使不传 Token，也能访问注册和登录接口
- ✅ 访问其他接口（如 `/api/v1/jobs`）仍然返回 401
...
```sql
-- 创建数据库
CREATE DATABASE IF NOT EXISTS smartats
...
CREATE TABLE `users` (
    ...
    `daily_ai_quota` INT NOT NULL DEFAULT 100,
...
-- 插入测试管理员账号
INSERT INTO `users` (`username`, `password`, `email`, `role`, `daily_ai_quota`, `status`)
VALUES (
    'admin',
...
**验收标准**：
- ✅ 在 DBeaver/DataGrip 中能看到 `smartats.users` 表
- ✅ 能查询到测试管理员账号
- ✅ 表结构与设计文档一致
...
**基础功能测试**：
1. ✅ 注册新用户成功（返回 200）
2. ✅ 重复注册时返回错误（10003 或 10004）
3. ✅ 登录成功返回 Token
4. ✅ 密码错误时返回统一提示（10006）
5. ✅ 用户不存在时返回统一提示（10006，与密码错误相同）

**安全测试**：
1. ✅ 未登录访问业务接口返回 401
2. ✅ 登录后带 Token 可访问（需实现 JWT 过滤器后）
3. ✅ Token 过期后返回 401（需实现 JWT 过滤器后）
4. ✅ Token 刷新功能正常（需实现 refresh 接口后）

### 6.5 常见问题排查

**问题 1：所有请求都返回 401**

**原因**：Spring Security 拦截了所有请求

**解决**：
```java
// 检查 SecurityConfig.java 是否正确配置
.requestMatchers("/api/v1/auth/register").permitAll()
.requestMatchers("/api/v1/auth/login").permitAll()
```

**问题 2：注册成功但登录返回"用户不存在"**

**原因**：数据库连接配置错误，或者没有提交事务

**解决**：
1. 检查 `application.yml` 中的数据库配置
2. 检查 `UserService.register()` 是否有 `@Transactional` 注解
3. 在数据库中直接查询用户数据确认

**问题 3：密码一直返回"用户名或密码错误"**
...
**解决**：
1. 确保注册时使用了 `passwordEncoder.encode()`
2. 确保登录时使用了 `passwordEncoder.matches()`
3. 检查数据库中的密码字段长度是否足够（BCrypt 需要 60 字符）

### 6.6 代码质量检查点

在完成认证模块后，检查以下几点：

- [ ] 使用 `LambdaQueryWrapper` 而不是字符串（类型安全）
- [ ] 密码使用 BCrypt 加密存储
- [ ] 统一错误提示（用户名不存在和密码错误返回相同提示）
